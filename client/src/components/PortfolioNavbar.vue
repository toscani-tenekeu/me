...
<template>
  <header class="sticky top-0 z-50 bg-bg border-b border-border">
    <div class="flex sm:grid sm:grid-cols-[1fr_auto_1fr] items-center h-[52px] max-w-[1100px] mx-auto px-6">
      ...
      <RouterLink
        to="/"
        class="flex items-center gap-2 min-w-0 overflow-hidden pr-4 sm:pr-0 text-fg no-underline hover:opacity-90 transition-opacity"
      >
        <span class="text-xs text-blue">~ $</span>
        <span class="block min-w-0 overflow-hidden font-mono text-[13px] text-fg">
          <TerminalTyper />
        </span>
      </RouterLink>
      ...
      <div class="ml-auto sm:ml-0 flex shrink-0 items-center gap-1.5 justify-end">
        ...
        <!-- Terminal button -->
        <button
          class="hidden sm:flex items-center gap-1 text-[11px] font-mono text-fg border border-border px-2.5 py-1 rounded-full hover:border-border-hi hover:bg-hover transition-colors cursor-pointer bg-transparent"
          title="Open terminal (Ctrl+K)"
          @click="openTerminal()"
        >
          <FontAwesomeIcon :icon="['fas', 'terminal']" class="text-[10px]" />
          Ctrl+K
        </button>
    ...
    <div v-if="menuOpen" class="sm:hidden flex flex-col border-t border-border-lo px-6 py-2 gap-0.5 bg-bg">
      ...
      <button
        class="text-left text-xs font-mono text-dim py-2 no-underline hover:text-link transition-colors cursor-pointer bg-transparent border-none"
        @click="menuOpen = false; openTerminal()"
      >
        <FontAwesomeIcon :icon="['fas', 'terminal']" class="text-[10px] mr-1.5" />
        terminal
      </button>
    </div>
  </header>
